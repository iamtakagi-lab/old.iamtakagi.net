<template>
  <div class="w-full relative flex flex-col justify-between">
    ...
    >
      ...
      <div class="relative">
        ...
        <input
          ...
          ref="search"
          v-model="q"
          class="block w-full pl-10 pr-3 py-2 truncate leading-5 placeholder-gray-500 border border-transparent text-gray-700 dark:text-white dark-focus:text-white focus:border-gray-300 dark-focus:border-gray-700 rounded-lg focus:outline-none focus:bg-white dark-focus:bg-gray-900 bg-gray-200 dark:bg-gray-800"
    ...
    >
      ...
      >
        <NuxtLink
          :to="{ name: 'index-slug', params: { slug: result.slug !== 'index' ? result.slug : undefined } }"
...
export default {
  ...
  watch: {
    async q (q) {
      ...
      this.searching = true
      this.results = await this.$content('articles' || 'index').sortBy('pos', 'desc').limit(12).search(q).fetch()
      this.searching = false
    }
  },
  mounted () {
